$(function(){
	...
	$(".cd-reply-btn").click(function(e){
		...
		if(reply.attr('class') == 'comment-group cd-none'){	//대댓글의 클래스명이 cd-none 이
			reply.attr('class', 'comment-group cd-block');		//대댓글의 클래스명을 cd-block로 변경
		}else{	//그롷지 않으면
			reply.attr('class', 'comment-group cd-none');	//대댓글의 클래스명을 cd-none로 변경 혹은 유지
		}
	});
	
	//댓글에 이미지 첨부시 이벤트
	$('.cd-img-file>img').click(function(){
