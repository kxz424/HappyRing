$(function(){
	...
	$(".inford-reply-btn").click(function(e){
	...
	$('.inford-comment-div .inford-img-file>img, .reply-send .inford-img-file>img, .inford-comment-comment-div .inford-img-file>img').click(function(){
		$(this).next().click();	//클릭이 발생한 태그의 다음 형제에게 클릭이벤트 부여
	});
	//이미지 첨부시 이미지 변경
	$('.inford-comment-div .inford-img-file>input:file, .reply-send .inford-img-file>input:file, .inford-comment-comment-div .inford-img-file>input:file').change(function(){
	...
	function url(value, tag) {
		...
		if(tagClass.match(/reply-send/)){
			...
			tagName = '.inford-comment-comment-div';
		}else if(tagClass.match(/thd-comment-div/)){
			tagName = '.inford-comment-div';
		}
		if (value.files && value.files[0]) {
			var img = $('<img class="thd-img-add" src="#"/>');	//이미지 태그 생성
			$(tagName+' .inford-img').html(img);	//생성한 이미지 태그를 원하는 태그 안으로 추가
			var reader = new FileReader();	//파일을 읽는 클래스의 생성자를 호출하여 객체 생성
			reader.onload = function(e){	//파일을 읽는 객체가 로드되었을때 실행할 함수 선언
				$(tagName+' .inford-img-add').attr('src', e.target.result);	//생성한 이미지 태그에 사용자가 선택한 이미지주소를 입력
			};
	...
	//etc 버튼 클릭시 이벤트 처리
	$('.inford-div-etc-buttons .btn').click(function(){
		if(!$(this).attr('class').match(/btn-active/)){		//클릭한 버튼이 현재 클릭된 상태인지 확인
			if($(this).attr('class').match(/btn-heart/)){		//클릭한 버튼의 클래스명에 btn-heart가 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-heart').addClass('btn-active');	//btn-heart 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-star/)){		//클릭한 버튼의 클래스명에 btn-star이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-star').addClass('btn-active');		//btn-star 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-share/)){		//클릭한 버튼의 클래스명에 btn-share가 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-share').addClass('btn-active');	//btn-share 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-correction/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-correction').addClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-comment/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-comment').addClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-bookmark/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-bookmark').addClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-pencil/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-pencil').addClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-edit/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-edit').addClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 추가
				
			}else if($(this).attr('class').match(/btn-trash/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-trash').addClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 추가
			...
			if($(this).attr('class').match(/btn-heart/)){		//클릭한 버튼의 클래스명에 btn-heart가 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-heart').removeClass('btn-active');		//btn-heart 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-star/)){		//클릭한 버튼의 클래스명에 btn-star이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-star').removeClass('btn-active');		//btn-star 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-share/)){		//클릭한 버튼의 클래스명에 btn-share가 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-share').removeClass('btn-active');		//btn-share 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-correction/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-correction').removeClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-comment/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-comment').removeClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-bookmark/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-bookmark').removeClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-pencil/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-pencil').removeClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-edit/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-edit').removeClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 삭제
				
			}else if($(this).attr('class').match(/btn-trash/)){		//클릭한 버튼의 클래스명에 btn-correction이 포함되어 있는지 확인
				$('.inford-div-etc-buttons .btn-trash').removeClass('btn-active');	//btn-correction 버튼에 클래스명 btn-active 삭제
	...
	//숨겨진 댓글창 여닫기
	$('.inford-div-etc-buttons .btn-comment').click(function(){
		if($('.inford-div-comment').css('display') == 'none'){		//댓글창이 숨겨져 있을 경우
			$('.inford-div-comment').css('display', 'block');		//댓글창을 보여줌
		}else{													//그렇지 않은 경우
			$('.inford-div-comment').css('display', 'none');		//댓글창을 숨김
		}
	});
